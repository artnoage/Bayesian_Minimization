...
class Loglikelihood2:
    ...
    def Loglikelihoodsimple(self,Sample):
        ...
        Barycenter = np.repeat(np.array([Barycenter]), NumberOfArchetypes, axis=0)
        ...
        # Spliting The plans
        Plans = np.array(np.split(Plans, NumberOfArchetypes, axis=0))

        # We apply the tranformation on the plans
        Plans = Transformation(Plans, "Plans", self.args[1])

        # Calculating the transportaion cost
        TransportationCost = np.sum(np.sum(Plans * CostMatrix, axis=1), axis=0)


        # Calculating the Margins of each plan.
        BarycenterMargin = np.sum(Plans.reshape(NumberOfArchetypes, NumberOfAtoms, NumberOfAtoms).transpose(0, 2, 1),
                                  axis=2)
        ArchetypeMargin = np.sum(Plans.reshape(NumberOfArchetypes, NumberOfAtoms, NumberOfAtoms), axis=2)
        ...
        if self.args[4] == "Yes":
            ReluData = - np.sum(np.minimum(Sample, np.zeros(len(Sample))), axis=0)
